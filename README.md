...
- 左側是類別，含「全部書籤」「最愛」，以及有未歸類書籤時才出現的「未分類」。
  這欄**預設是收起來的**：滑鼠移到畫面左緣或左上角的 ☰ 就會浮出來蓋在內容上，
  移開後自動收回。側欄裡的 📌 可以把它釘住，釘住後改成固定佔一欄、內容往右讓位。
  釘選狀態記在 localStorage。觸控裝置沒有 hover，改用點 ☰ 開關。
  拖曳書籤時把游標帶到左緣，側欄一樣會浮出來讓你放。
...
`index.html` 引用資產時帶了版本參數（`assets/app.js?v=10`）。GitHub Pages 對靜態檔的
